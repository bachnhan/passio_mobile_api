...
namespace Wisky.SkyAdmin.Manage.Areas.SysAdmin.Controllers
{
    public class ReDateReportController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var brands = brandApi.GetActive().Select(q => new SelectListItem { Value = q.Id.ToString(), Text = q.BrandName });
        ...
        public ActionResult ViewReport(string sDate, string eDate, /*string date,*/ int brandId, int storeId)
        {
            try
            {
                ...
                IEnumerable<PaymentReport> paymentReports;
                var paymentQuery = paymentReportService.Get(q => q.Date >= fromDate && q.Date <= toDate);
                if(storeId != 0)
                {
                    paymentReports = paymentQuery.Where(q => q.Store.BrandId == brandId).ToList();
                }
                else
                {
                    paymentReports = paymentQuery.Where(q => q.StoreID == storeId).ToList();
                }
                ...
                foreach (var item in reports)
                {
                    ...
                    var resultReport = new DateCheckReport
                    {
                        ...
                        TotalPayments = oldPaymentReport == null ? 0 : oldPaymentReport.BankAmount + oldPaymentReport.CashAmount + oldPaymentReport.DebtAmount + oldPaymentReport.PayDebtAmount + oldPaymentReport.MemberCardAmount + oldPaymentReport.OtherAmount + oldPaymentReport.ReceiptAmount+ oldPaymentReport.SpendAmount,
                        RealTotalPayments = checkPaymentReport.BankAmount + checkPaymentReport.CashAmount + checkPaymentReport.DebtAmount + checkPaymentReport.PayDebtAmount + checkPaymentReport.MemberCardAmount + checkPaymentReport.OtherAmount+ checkPaymentReport.ReceiptAmount + checkPaymentReport.SpendAmount,

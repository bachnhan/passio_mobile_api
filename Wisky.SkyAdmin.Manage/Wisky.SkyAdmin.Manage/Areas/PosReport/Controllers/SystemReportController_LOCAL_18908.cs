...
namespace Wisky.SkyAdmin.Manage.Areas.PosReport.Controllers
{
    ...
    public class SystemReportController : DomainBasedController
    {
        ...
        public async System.Threading.Tasks.Task<JsonResult> ExportCategoryAllStoreToExcel(string startTime, string endTime, int catetoryId, int brandId)
        {
            //var brandId = 1;
            var productCategoryApi = new ProductCategoryApi();
            ...
            IEnumerable<ProductCategoryViewModel> listCategory = catetoryId == 0 ? productCategoryApi.GetProductCategories(brandId).Where(a => a.IsDisplayed).ToList() :
                productCategoryApi.GetProductCategories(brandId).Where(a => a.IsDisplayed && a.CateID == catetoryId).ToList();
            ...
            if (startTime == today && endTime == today)
            {
                ...
                foreach (var itemCate in listCategory)
                {
                    var quantity = result.Where(a => a.CategoryId == itemCate.CateID).Sum(b => b.Quantity);
                    var total = result.Where(a => a.CategoryId == itemCate.CateID).Sum(b => b.ToTal);
                    categories.Add(new Tuple<string, int, int>(itemCate.CateName, quantity, (int)total));
                    totalProduct += quantity;
                }
            ...
            var categoryName = (catetoryId == 0 ? "TấtCảNgànhHàng" : (await productCategoryApi.GetProductCategoryById(catetoryId)).CateName);
            ...
            string fileName = "BáoCáoNgànhHàng_" + categoryName + "_TấtCảCửaHàng_HệThống" + dateRange;
            bool success = false;
        ...
        public async System.Threading.Tasks.Task<ActionResult> ExportCategoryOneStoreToExcel(string startTime, string endTime, int storeIdReport, int catetoryId, int brandId)
        {
            var productCategoryApi = new ProductCategoryApi();
            var dateProductApi = new DateProductApi();
            var storeApi = new StoreApi();
            ...
            var listCategory = catetoryId == 0 ? productCategoryApi.GetProductCategories(brandId).Where(a => a.IsDisplayed).ToList() :
                productCategoryApi.GetProductCategories(brandId).Where(a => a.IsDisplayed && a.CateID == catetoryId).ToList();
            ...
            if (startTime == today && endTime == today)
            {
                ...
                foreach (var itemCat in listCategory)
                {
                    ...
                    if (total != null)
                        categories.Add(new Tuple<string, int, int>(itemCat.CateName, quantity, (int)total));
                }
            }
            else
            {
            ...
            var success = ExportToExcelExtensions.ExportToExcel(header, listProduct, fileName);
        ...
        public List<dynamic> GetListRevenueReport(string startTime, string endTime, int brandId)
        {
            ...
            //list = listDateReport.Select(a => new
            //{
            //    No = a.StartTime,
        ...
        public ActionResult IndexDayOfWeekReport(string storeid, int brandId)
        {
            var storeApi = new StoreApi();
            var store = storeApi.GetActive().Where(q => q.Type == 5 && q.BrandId == brandId);
        ...
        public ActionResult ExportProductCategoryAllStoreToExcel(string startTime, string endTime, int? startHour, int? endHour, int? brandId)
        {
            ...
            var list = fillterList.Select(a => new
            {
                a = a.ProductName,
                b = a.CateName,
                c = string.Format(CultureInfo.InvariantCulture, "{0:0,0}", a.Quantity),
                d = string.Format(CultureInfo.InvariantCulture, "{0:0,0}", a.TotalPrice),
                e = a.ProductId,
            }).ToList();

            List<string> header = new List<string>();
            header.Add("STT;1;1");
            header.Add("Tên Sản phẩm;1;1");
            header.Add("Ngành hàng;1;1");
            header.Add("Số lượng;1;1");
            header.Add("Tổng tiền;1;1");

            string fileName = "Báo cáo theo sản phẩm";
